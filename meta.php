<?php
/**
 * @package Meta Content
 * @author Mervin Praison
 * @version 1.1
 */
/*
    Plugin Name: Meta Content
    Plugin URI: https://mer.vin/meta/
    ...
    Text Domain: meta-content-types
...
if ( function_exists('add_action') ) {
    ...
    // PLUGINDIR constant is deprecated, using WP_PLUGIN_DIR instead
...
if ( !class_exists( 'MetaContentClass' ) ) {
    class MetaContentClass {
        ...
        function enqueue_script() {
            wp_enqueue_script( 'tinymce4dt', WP_PLUGIN_URL . '/' . MP_ME_BASEDIR . '/js/script.js', array('jquery'), '1.1', true );
        }
        // admin init
        ...
        function is_page_preview() {
            $id = isset($_GET['preview_id']) ? absint($_GET['preview_id']) : 0;
            if ($id == 0 && isset($_GET['post_id'])) {
        ...
        function on_wp_insert_post($id) {
            // Verify nonce
            if (!isset($_POST['meta_content_nonce']) || !wp_verify_nonce($_POST['meta_content_nonce'], 'meta_content_save')) {
                return;
            }
            
            if ( !isset($id) && isset($_REQUEST['post_ID']) )
                $id = absint($_REQUEST['post_ID']);
            if ( $this->is_page_preview() && !isset($id) && isset($_GET['preview_id']) )
                $id = absint($_GET['preview_id']);
            if ( !current_user_can('edit_post', $id) )
                return;
            
            if ( isset($_POST['dt-meta-content']) && $_POST['dt-meta-content'] != '' )
                $this->data['meta-content'] = wp_kses_post(wp_unslash($_POST['dt-meta-content']));
        ...
        function meta_box($data) {
            ...
            wp_nonce_field('meta_content_save', 'meta_content_nonce');
            ?>
            <table id="dt-page-definition" width="100%" cellspacing="5px">
                
                <tr valign="top">
                    <td><label for="dt-meta-content"><?php esc_html_e( 'Meta Content:', 'meta-content-types' ); ?></label></td>
                    <td><textarea cols="16" rows="5" id="dt-meta-content" name="dt-meta-content" class="meta-content form-input-tip code" size="20" autocomplete="off" tabindex="6" style="width:90%"/><?php echo isset($value['meta-content']) ? esc_textarea($value['meta-content']) : ''; ?></textarea>
    ...
    function getmetacontent($id='', $type = '', $string = '') {
        ...
        $type = "meta-content";
        // Output is already sanitized when saved with wp_kses_post()
